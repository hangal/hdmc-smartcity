...
with open('all_works_2014_15.csv', 'rU', encoding="cp1252", errors="ignore") as f:
    reader = csv.reader(f)
    f_out = open("kannada2.csv", encoding='utf-8', mode='w')
    #reader.next()
    for row in reader:
        convertedText=process_line(row[5])   
        f_out.write(convertedText)
        break 

f_out.close()
"""

input = open('all_works_2014_15.csv', 'rU')
with open('all_works_201415_k.csv', 'w', encoding='utf-8', newline='') as output:
